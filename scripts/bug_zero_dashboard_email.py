...
def fetch_data():
    ...
    # Count in Verification step
    cursor.execute(f"""
        SELECT COUNT(*) as cnt FROM tbl_ElvisSR
        WHERE {BUG_ZERO_WHERE} AND `TicketStepID` = 'Verifying'
    """)
    verification_count = cursor.fetchone()["cnt"]
...
def build_html(data):
    ...
    for i, dom in enumerate(split_domains):
        ...
        for d in last5:
            ...
            in_val = str(iv) if iv else dot
            out_val = str(ov) if ov else dot
            split_day_cells += f'<td style="padding:4px 4px;border-bottom:1px solid #eee;font-size:12px;text-align:center;background:#fff5f5;color:{in_color};{in_w}{tf}border-left:2px solid #e0e0e0;">{in_val}</td>'
            split_day_cells += f'<td style="padding:4px 4px;border-bottom:1px solid #eee;font-size:12px;text-align:center;background:#f0fff0;color:{out_color};{out_w}{tf}">{out_val}</td>'
...
<tr><td style="padding:0 28px 12px 28px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border:2px solid #bdc3c7;border-radius:6px;border-collapse:collapse;">
        <tr style="background:#1a5276;">
            <td rowspan="2" style="padding:8px 12px;font-size:13px;font-weight:600;color:#fff;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">Domain</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">Total</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">TOP+A</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">B+C Always</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">B+C Sometimes</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:1px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">B+C Once</td>
            <td rowspan="2" style="padding:8px 8px;font-size:13px;font-weight:600;color:#fff;text-align:center;border-right:2px solid #2980b9;font-family:'Segoe UI',Calibri,Arial,sans-serif;">Repro</td>
            {date_header_top}
        </tr>
        <tr style="background:#1a5276;">{date_header_sub}</tr>
        {split_rows}
        {split_total_row}
    </table>
</td></tr>
